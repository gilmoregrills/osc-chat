import { Master, Waveform, Transport, Time, Draw } from "tone";

// const canvasWidth = 512; // This constant doesn't seem to be used. Remove?

const makeCanvas = (): HTMLCanvasElement | null => {
  const div = document.getElementById("waveform") as HTMLDivElement | null;
  if (!div) {
...
const nearestPowerOf2 = (n: number): number => {
  if (n <= 0) return 32; // Default to a small power of 2 if input is invalid
  return Math.pow(2, Math.floor(Math.log2(n)));
};

export const startWaveformLoop = (): void => {
  ...
  Master.connect(waveform); // Connect Master output to the waveform analyzer

  const repeat = (time: number): void => { // Changed type Time to number
  ...
  Transport.scheduleRepeat(repeat, "0.1"); // Schedule the repeat function every 0.1 seconds
};
